...
const WeeklyActivityTracker = () => {
    ...
    const approveinst = async() =>{
        ...
        setAdding(true);
        const result = await postapicallhelper("Liason/UniversitySupervisorApproveRejectStudentWeeklyActivity", datatosave);
        setAdding(false);
    ...
    const fetchalldata = async () => {
        // setLoading(true);
        // const result = await getapicallhelper("liason/GetUniversitySupervisorStudentWeeklyWorks", {});
        // if(result.success == true){
        //     setPageData(result?.data?.weeklyLogData);
        //     console.log("GetUniversitySupervisorStudentWeeklyWorks", result?.data?.weeklyLogData)
        // }
        // setLoading(false);
        ...
        setPageData([
            {
                ...
                weekEndDate: "2020-01-01",
                newToolsUsed:"Some tools used",
                newSkillsLearned:"Some skills learned",
                newKnowledgeAcquired:"Some knowledge acquired",
                outstandingQuestion:"Some question"
            }
        ]);
    };
    ...
    return (
        <div>
            <div style={titles}>
            <AccessInfo style={{ margin: "0 20px 0 0" }}>Unapproved Students Weekly Activity Trackers</AccessInfo>
            </div>
...

            {
                ...
                <ul className="responsive-table">
                    ...
                    <div className="col col-d-2">Week Start At</div>
                    <div className="col col-d-3">Week End At</div>
                    ...
                    <TRow
                        ...
                        title2 = {moment(data?.weekStartDate).format('MMMM Do YYYY')}
                        title3 = {moment(data?.weekEndDate).format('MMMM Do YYYY')}
                        title4 = {"Pending"}
                        rowClicked = {(datapassed)=>{
                            ...
                            setFullDataInfo([
                                ...
                                {
                                    title:"Week Start At",
                                    content:`${moment(data?.weekStartDate).format('MMMM Do YYYY')}`
                                },
                                {
                                    title:"Week End At",
                                    content:`${moment(data?.weekEndDate).format('MMMM Do YYYY')}`
                                },
                                {
                                    title:"New Tools Used",
                                    content:data?.newToolsUsed
                                },
                                {
                                    title:"New Skills Learned",
                                    content:data?.newSkillsLearned
                                },
                                {
                                    title:"New Knowledge Acquired",
                                    content:data?.newKnowledgeAcquired
                                },
                                {
                                    title:"Outstanding Question",
                                    content:data?.outstandingQuestion
                                }
                            ])
                            setPopup(true);
                        }}

import React, { useEffect, useState } from "react";
import AnimateHeight from "react-animate-height";
import TRow from "../../components/General/TRow";
import {AccessInfo,FormInput,DataListInput} from "../../components/styles/Access";
import {DashSearchContainer} from "../../components/styles/Dashboard";
import { GlobalButton } from "../../components/styles/Global";
import { colors } from "../../utils/colors";
import "../../components/styles/DuesTable.scss";
import "../../components/styles/join.css";
import { ClipLoader } from "react-spinners";
import { MdError } from "react-icons/md";
import { IoNotificationsCircleSharp } from "react-icons/io5";
import { useRef } from "react";
import { isAfter, isValidText } from "../../utils/helpers";
import { SearchFilter } from "../../components/General/SearchFilter";
import { getapicallhelper, postapicallhelper } from "../../network/ApiHelper";
import Select from "react-select";
import { colourStyles } from "../../components/styles/Select2";
import PopupView from "../../components/General/PopupView";
import { ModalDialog } from "../../components/General/ActionModal";
// import { PendingFullContainer, PendingFullDiv } from "../../components/styles/PendingRegistrations";
import { Confirm, Modal, Button, Header, Image,  Icon, Table, List } from 'semantic-ui-react'

const InternshipOpportunities = () => {
    const [modaldialog, setmodaldialog] = useState(false);
    const [modaldialogbtn1Text, setmodaldialogbtn1Text] = useState("");
    const [modaldialogbtn2Text, setmodaldialogbtn2Text] = useState(null);
    const [modaldialogbtn2Action, setmodaldialogbtn2Action] = useState(null);
    const [triggerchage, settriggerchange] = useState(false);
    const [modalDialogContent, setModalDialogContent] = useState({
        content:"",
        type:"",
        headertitle:""
    });

    const [popup, setPopup] = useState(false);
    const [loading, setLoading] = useState(false);
    const [pageData, setPageData] = useState([]);
    const [message, setMessage] = useState({
        content:"",
        type:""
    });

    const [formData, setFormData] = useState({});
    const formRef = useRef();

    const [fulldatainfo, setFullDataInfo] = useState([]);
    const [selectedData, setSelectedData] = useState({});

    useEffect(()=>{
        if(modaldialogbtn2Action=="RSC"){
        }
    }, [triggerchage])


    const gettotalslots = (passeddata) =>{
        let noofslots = 0;
        if(Array.isArray(passeddata)== true){
            passeddata.forEach(element => {
                noofslots = noofslots + Number(element.numberOfSlots)
            });
        }
        return `${noofslots} slot(s)`;
    }
  
    useEffect(() => {
        fetchalldata();
    }, []);


    const handleChange = (event) => {
		setMessage({content:"",type:""})
		let objectkey = event.target.name;
		let objectvalue = event.target.value;
		let oldData = Object.assign({}, formData);
		oldData[objectkey] = objectvalue;
		setFormData(oldData);
	};

    const fetchalldata = async () => {
        setPageData([]);
        // setLoading(true);
        // const result = await getapicallhelper("liason/GetInternshipOpportunities", {});
        // setPageData(result?.data?.internshipOpportunities);
        // console.log("formatresultlist", result?.data?.internshipOpportunities)
        // setLoading(false);
    };

    const datatoxtract = (array) =>{
        let datatoreturn = [];
        for (let index = 0; index < array.length; index++) {
            const data = array[index];
            datatoreturn.push({
                "Company Name":`${data?.industryName}`,
                "Region":data?.region,
                "Branch":`${data?.branch}`,
                "Total Slots": `${gettotalslots(data?.slotRequests)}`,
                "Contact Person":`${data?.contactPerson}` ,
                "Contact Person Email":`${data?.contactPersonEmail}` ,
                "Contact Person Phone Number": `${data?.contactPersonPhoneNumber}`,
                "Industry Category": `${data?.category}`,
                "Industry Operating City": `${data?.operatingCity}`
            })
        }
        return datatoreturn
    }

    

    return (
        <div>
            <div style={titles}>
            <AccessInfo style={{ margin: "0 20px 0 0" }}>Internship Opportunities</AccessInfo>
            </div>

            {
                isValidText(message.content) &&
                <>
                <DashSearchContainer
                style={{
                    margin: "10px 0",
                    padding: 10,
                    justifyContent: "space-between",
                    alignItems: "center",
                    backgroundColor: `${message.type == "err" ? colors.danger : colors.success}`,
                    border: "none",
                    color: "white",
                }}
                >
                {message.content}
                <MdError color={"white"} size={20} />
                </DashSearchContainer>
                </>
            }
            

            {
                loading &&
                <DashSearchContainer
                style={{
                    margin: "10px 0",
                    padding: 10,
                    justifyContent: "space-between",
                    alignItems: "center",
                    border: "1px solid rgba(0, 0, 0, 0.09)",
                }}
                >
                Loading..
                <ClipLoader color={colors.primary} loading={true} size={15} />
                </DashSearchContainer>
            }


            {
                (loading == false && pageData.length == 0 && message.type != "err") && 
                <DashSearchContainer style={nodatayet}>
                Currently don't have any data yet!
                <IoNotificationsCircleSharp color={colors.primary} size={30} />
                </DashSearchContainer>
            }

            

            {
                (loading == false && pageData.length > 0) &&
                <>
                <SearchFilter
                    showexcelbtn = {true}
                    excelname = "Internship Opportunities.csv"
                    exceldata = {datatoxtract(pageData)}
                />
                <ul className="responsive-table">
                    <li className="table-header">
                    <div className="col col-d-1">Company Name</div>
                    <div className="col col-d-2">Region</div>
                    <div className="col col-d-3">Branch</div>
                    <div className="col col-d-4">Total slots</div>
                    </li>
                    
                    {pageData.map((data, index) => (
                    <TRow
                        title1 = {data?.industryName}
                        title2 = {data?.region}
                        title3 = {data?.branch}
                        title4 = {gettotalslots(data?.slotRequests)}
                        rowClicked = {(datapassed)=>{
                            setMessage({content:"",type:""});
                            setSelectedData(data);
                            setFullDataInfo([
                                {
                                    title:"Company Name",
                                    content:`${data?.industryName}` 
                                },
                                {
                                    title:"Contact Person",
                                    content:`${data?.contactPerson}` 
                                },
                                {
                                    title:"Contact Person Email",
                                    content:`${data?.contactPersonEmail}` 
                                },
                                {
                                    title:"Contact Person Phone Number",
                                    content:`${data?.contactPersonPhoneNumber}` 
                                },
                                {
                                    title:"Industry Category",
                                    content:`${data?.category}` 
                                },
                                {
                                    title:"Industry Region",
                                    content:`${data?.region}` 
                                },
                                {
                                    title:"Industry Branch",
                                    content:`${data?.branch}` 
                                },
                                {
                                    title:"Industry Operating City",
                                    content:`${data?.operatingCity}` 
                                },
                            ])
                            setPopup(true);
                        }}
                        hideviewmore = {true}
                        fullinfo = {[]}
                        data={data} 
                        key={index}
                    />
                    ))}
                </ul>
                </>
            }


            <PopupView payView={popup} setPayView={setPopup}>


                <div style={{minWidth:600}}>
                    <Table celled striped>
                        <Table.Header>
                            <Table.Row>
                                <Table.HeaderCell colSpan='2'>{fulldatainfo[0]?.content}</Table.HeaderCell>
                            </Table.Row>
                        </Table.Header>

                        <Table.Body>
                            {
                                fulldatainfo?.map((eachr, index)=>(
                                    <Table.Row key={index}>
                                        <Table.Cell>{eachr.title}</Table.Cell>
                                        <Table.Cell>{eachr.content}</Table.Cell>
                                    </Table.Row>
                                ))
                            }
                            <Table.Row>
                                <Table.Cell>Slots</Table.Cell>
                                <Table.Cell>
                                    {
                                        selectedData?.slotRequests?.map((eachrow, index)=>(
                                            <div key={index}>
                                                <p>{`${eachrow.course} - ${eachrow.numberOfSlots} slot(s)`}</p>              
                                            </div>
                                        ))
                                    }
                                </Table.Cell>
                            </Table.Row>

                        </Table.Body>
                    </Table>
                </div>
                
            </PopupView>




            <ModalDialog
                modalVisible ={modaldialog}
                setmodaldialog = {setmodaldialog}
                btn1Text = {modaldialogbtn1Text}
                btn2Text = {modaldialogbtn2Text}
                btn1Pressed = {()=>{
                    setmodaldialog(false)
                }}
                btn2Pressed = {()=>{
                    setmodaldialog(false)
                    settriggerchange(!triggerchage);
                }}
                modalDialogContent = {modalDialogContent}
            />

        </div>
    );
};

export default InternshipOpportunities;

const titles = {
    margin: "15px 0",
    textAlign: "center",
    display: "flex",
    justifyContent: "center",
    alignItems: "center",
}

const btncancel = {
    margin: 0,
    marginRight: 20,
    borderRadius: 5,
    padding: "10px 20px",
    width: 100,
}

const btnsuccess = {
    margin: 0,
    borderRadius: 5,
    padding: "10px 20px",
    width: 100,
}

const generalbtn = {
    margin: 0,
    borderRadius: 5,
    padding: "10px 20px",
    width: 100,
}

const nodatayet = {
    margin: "10px 0",
    padding: 10,
    justifyContent: "space-between",
    alignItems: "center",
    border: "1px solid rgba(0, 0, 0, 0.09)",
    backgroundColor: "#f8f8fa",
    fontSize: 14,
}
...
const AssignStudentSupervisor = () => {
    ...
    const approveinst = async() =>{
        let errorList = [];
        if(isValidText(formData.supervisorId) == false){
            errorList.push("Please select a university supervisor");
        }
        ...
        setAdding(true);
        const result = await postapicallhelper("Liason/AssignStudentUniversitySupervisor", datatosave);
        setAdding(false);
    ...
    const fetchalldata = async () => {
        ...
        const result6 = await getapicallhelper("Liason/GetUniversitySupervisors", {});
    ...
    const getsupervisorname = (data) =>{
        if(isValidText(data?.universitySupervisor?.firstName)){
            return `${data?.universitySupervisor?.firstName} ${data?.universitySupervisor?.lastName}`
        }
        else{
            return "Not assigned"
        }
    ...
    const datatoxtract = (array) =>{
        ...
        for (let index = 0; index < array.length; index++) {
            ...
            datatoreturn.push({
                ...
                "Student Phone Number": data?.phoneNumber,
                "Assigned To": `${getsupervisorname(data)}`
            })
        }
        return datatoreturn
    }
    ...
    return (
        <div>
            <div style={titles}>
            <AccessInfo style={{ margin: "0 20px 0 0" }}>Assign Students To Supervisor</AccessInfo>
            </div>
...

            {
                ...
                <ul className="responsive-table">
                    ...
                    <div className="col col-d-4">Assigned To</div>
                    </li>
                    ...
                    <TRow
                        ...
                        rowClicked = {(datapassed)=>{
                            ...
                            setFullDataInfo([
                                ...
                                {
                                    title:"Assigned To",
                                    content:`${getsupervisorname(data)}`
                                },
                            ])
                            setPopup(true);
                        }}
            ...
            <PopupView payView={popup} setPayView={setPopup}>
                ...
                <Select
                    ...
                    onChange={(selval)=>{
                        let oldData = Object.assign({}, formData);
                        oldData["supervisorId"] = selval.value;
                        setFormData(oldData);
                    }}
                />
...
export default AssignStudentSupervisor;
